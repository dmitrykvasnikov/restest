...
- `go.mod`, `cmd/restest/main.go`, graceful shutdown on SIGTERM.
- Config from environment; fail loudly at startup on anything missing.
- `log/slog` JSON handler, request id in context, logged on every request.
- pgx connection pool; goose migrations embedded via `go:embed` and applied at startup.
- `docker-compose.yml` with Postgres 17 and the app; multi-stage Dockerfile to distroless.
...
## M1 — Accounts and projects

- Register, log in, log out. Argon2id hashing with tuned parameters.
- `scs` sessions in Postgres; `HttpOnly`, `Secure`, `SameSite=Lax`.
- CSRF via `nosurf` on every mutating form.
- Project CRUD. Slug validation against the format constraint, plus a reserved-word list
  (`api`, `admin`, `demo`, `m`, `static`, `healthz`).
- Base layout: Tailwind, HTMX, page shell, flash messages.

**Done when:** a new user can register, log in and create a project.

## M2 — Static mocks — *first useful milestone*

- Endpoint CRUD in the UI, with CodeMirror for the response body.
- **The route matcher.** In-memory radix trie built from the database, held behind an
  `RWMutex`, rebuilt on any endpoint change. Literal segments outrank parameter segments, so
  `/users/me` beats `/users/{id}`.
- `/m/{slug}/…` handler: match, apply `delay_ms`, write status, headers and body.
- Unmatched requests return 404 with a JSON body listing the nearest defined routes.
- Method fallthrough: a path that exists under a different verb returns 405, not 404.
...
## M3 — Stateful collections — *satisfies the core of task.md*

- Collection CRUD; seed edited as a JSON array.
- One `collection` endpoint expands to six routes rooted at its path.
- List: `_page`, `_limit`, `_sort`, `_order`, plus `field=value` filters served by the GIN index.
- Create allocates ids via `UPDATE collections SET next_serial = next_serial + 1 RETURNING`,
  or generates a uuid, per `id_strategy`.
- `PUT` replaces, `PATCH` shallow-merges, `DELETE` returns 204.
- Reset to seed, in the UI and at
...
## M6 — API tokens and management API
